...
def _op_in_list(item_val: str, rule_val) -> bool:
    allowed = {value.lower() for value in _rule_values(rule_val)}
...
def _op_in_user_group(item_val: str, _rule_val: str, *, word_list: list[str] | None = None) -> bool:
    """用户组使用去空格、忽略大小写后的精确匹配。"""
    if not word_list:
        return False
    target = item_val.casefold().strip()
    return target in {str(value).casefold().strip() for value in word_list if str(value).strip()}


def _op_not_in_user_group(item_val: str, _rule_val: str, *, word_list: list[str] | None = None) -> bool:
    """空组或缺失组保持不匹配；只有非空组才允许反向判断。"""
    if not word_list:
        return False
    return not _op_in_user_group(item_val, _rule_val, word_list=word_list)


def _op_starts_with_content_group(
    item_val: str, _rule_val: str, *, word_list: list[str] | None = None,
) -> bool:
    if not word_list:
        return False
    target = item_val.casefold()
    return any(
        prefix and target.startswith(str(prefix).casefold())
        for prefix in word_list
    )


def _op_contains_content_group(
    item_val: str, _rule_val: str, *, word_list: list[str] | None = None,
) -> bool:
    if not word_list:
        return False
    target = item_val.casefold()
    return any(
        part and str(part).casefold() in target
        for part in word_list
    )


def _op_ends_with_content_group(
    item_val: str, _rule_val: str, *, word_list: list[str] | None = None,
) -> bool:
    if not word_list:
        return False
    target = item_val.casefold()
    return any(
        suffix and target.endswith(str(suffix).casefold())
        for suffix in word_list
    )


def _op_not_starts_with_content_group(
...
OPERATORS = {
    ...
    "in_user_group": _op_in_user_group,
    "not_in_user_group": _op_not_in_user_group,
    "starts_with_content_group": _op_starts_with_content_group,
    "not_starts_with_content_group": _op_not_starts_with_content_group,
    "ends_with_content_group": _op_ends_with_content_group,
    "not_ends_with_content_group": _op_not_ends_with_content_group,
    "contains_content_group": _op_contains_content_group,
    "not_contains_content_group": _op_not_contains_content_group,
}
...
def _named_group(groups, name: str) -> list[str] | None:
    ...
    for group_name, values in groups.items():
        if str(group_name).casefold() == target:
            return values
    return None


def _check_condition(condition: dict, item: dict, word_lists: dict) -> bool:
    """检查单个条件是否匹配。"""
    ...
    if op in ("in_whitelist", "in_blacklist"):
        ...
        legacy_group = DEFAULT_USER_GROUP if op == "in_whitelist" else RESTRICTED_USER_GROUP
        word_list = _named_group(word_lists.get("user_groups", {}), legacy_group)
        if word_list is None:
            word_list = word_lists.get(list_name, [])
        return op_func(item_val, value, word_list=word_list)
    if op == "starts_with_content_wl":
        word_list = _named_group(word_lists.get("content_groups", {}), DEFAULT_CONTENT_GROUP)
        if word_list is None:
            word_list = word_lists.get("content_whitelist", [])
        return op_func(item_val, value, word_list=word_list)
    if op in ("in_user_group", "not_in_user_group"):
        word_list = _named_group(word_lists.get("user_groups", {}), str(value).strip())
        return op_func(item_val, value, word_list=word_list)
    if op in (
        ...
        word_list = _named_group(word_lists.get("content_groups", {}), str(value).strip())
        return op_func(item_val, value, word_list=word_list)

    return op_func(item_val, value)


def _match_rule_group(rule: dict, item: dict, word_lists: dict) -> bool:
...
def match_rules(
    ...
    """兼容接口：返回 (action, matched_rule_name)。"""
    matched = match_rules_detailed(item, rules, word_lists)
    return matched["action"], matched["rule_name"]


def match_rules_detailed(
    item: dict,
    rules: dict,
    word_lists: dict[str, list[str]] | None = None,
) -> dict:
    """返回动作、规则名和完整规则；通知规则不再充当动作规则。"""
    ...
    for rule in rules.get("auto_reject", []):
        if _match_rule_group(rule, item, word_lists):
            return {"action": "reject", "rule_name": rule.get("name", ""), "rule": rule}

    # auto_approve
    for rule in rules.get("auto_approve", []):
        if _match_rule_group(rule, item, word_lists):
            return {"action": "approve", "rule_name": rule.get("name", ""), "rule": rule}

    return {"action": "", "rule_name": "", "rule": None}


def closest_rule_mismatch(
    item: dict,
    rules: dict,
    word_lists: dict | None = None,
) -> str:
    """解释最接近但未命中的动作规则，供只读预览诊断配置问题。"""
    word_lists = word_lists or {}
    candidates = []
    for rule_type, action_label in (
        ("auto_reject", "拒签"),
        ("auto_approve", "签核"),
    ):
        for order, rule in enumerate(rules.get(rule_type, [])):
            conditions = [
                condition
                for condition in rule.get("conditions", [])
            ...
            failed = [
                ...
                if not _check_condition(condition, item, word_lists)
            ]
            logic = str(rule.get("logic", "AND")).upper()
            if (logic == "OR" and len(failed) != len(conditions)) or (
                logic != "OR" and not failed
            ):
                continue
            candidates.append((len(failed), order, action_label, rule, failed))
    if not candidates:
        return ""
    _count, _order, action_label, rule, failed = min(
        candidates,
        key=lambda value: (value[0], value[1]),
    )
    details = []
    for condition in failed[:2]:
        field = str(condition.get("field", ""))
        actual = _get_field_value(item, field).strip() or "空"
        expected = str(condition.get("value", "")).strip() or "无需值"
        details.append(
            f"{field}实际“{actual[:30]}”未满足 "
            f"{condition.get('op', '')} “{expected[:30]}”"
        )
    suffix = "；另有未满足条件" if len(failed) > 2 else ""
    return (
        f"最接近{action_label}规则「{rule.get('name', '未命名规则')}」："
        + "；".join(details)
        + suffix
    )
...
def load_word_lists(base_dir: str = ".", rules: dict | None = None) -> dict:
    """加载命名组与旧名单；命名组优先，旧名单保持兼容。
    ...
    groups = group_values_for_rules(skill, rules)
    lists["user_groups"] = {
        name: [str(value).casefold() for value in values]
        for name, values in groups.get("user_groups", {}).items()
    }
    lists["content_groups"] = {
        name: [str(value).casefold() for value in values]
        for name, values in groups.get("content_groups", {}).items()
    }
