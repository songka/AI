# 019ff929-1e57-79b3-ab3c-11a0d53daae3

- 会话 ID：`019ff929-1e57-79b3-ab3c-11a0d53daae3`
- 最后更新：2026-08-13T03:27:29.414416+00:00
- 来源：Codex 本地会话
...
我会用 `csharp-async`、`dotnet-best-practices` 和 `mvvm-toolkit` 约束这一工作包：重点处理刷新/选中会话的取消、异常边界和 WPF 列表虚拟化；所有改动只落在 UI 项目。

## 助手

现状确认：接口本身已经支持 `CancellationToken`，但 UI 没有传递；预览仍由外层 `ScrollViewer + ItemsControl` 一次性生成全部消息。接下来会把取消生命周期封装在 ViewModel，并让窗口关闭时主动释放未完成任务。

## 助手

核心改动已完成：快速切换会话会真正取消旧文件解析；刷新会取消预览并把各 Agent 错误分别记录；消息预览已启用回收式虚拟化，并分批把大量消息交给 UI，避免长时间独占界面线程。现在进行编译和静态检查。
...
Phase 2 的 `.ai-project` 会作为完全独立的新项目实现。我会继续使用 .NET 异步规范，并加入 `csharp-xunit` 指导故障、取消、路径穿越和冲突测试；所有写入严格限制在两个新目录。

## 助手

接口范围已经定下：归档仅收集显式项目目录；会话归档必须由调用者逐个显式传入。恢复只还原 `project/**`，内嵌 `sessions/*.ai-session` 不会被写入项目目录，以免把 Agent/会话数据混入工程。恢复策略为默认冲突、跳过、保留两份三种。
