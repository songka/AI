# 019ff912-5da3-7a50-8bf3-daeecd051978

- 会话 ID：`019ff912-5da3-7a50-8bf3-daeecd051978`
- 最后更新：2026-08-13T03:02:38.295361+00:00
- 来源：Codex 本地会话
...
我会先核对当前格式与导出代码，再结合公开仓库筛选可直接落地的 skills/agent 工作流；这一轮只做只读调研，不改文件。由于需要在线检索，我会使用浏览与技能安装目录的既有规范来过滤不安全或许可证不明的候选。

## 助手

仓库现状已经清楚：USF 与 ZIP 目录约定已有雏形，但三个适配器都只覆盖“文本主路径”，导出/打包尚未落地；OpenCode 仍是临时 JSONL 假设。接下来我会优先查官方/许可证清晰的候选，并把“可安装 skill”与“仅适合作为代码参考的项目”分开。

## 助手

筛选结果已经收敛：最值得引入的是 OpenCode SQLite 只读工具包、Codex 会话导出 skill，以及测试审查/阶段门禁 skills。几个“看起来相关但无许可证”的 Claude 导出项目我会明确排除，避免后面形成授权债务。
...
我会补齐三个 skill 的精确安装路径和脚本行为，重点区分“脚本本身联网”与“安装阶段需要联网”；仅给采用结论，不改项目或本机 skills。
