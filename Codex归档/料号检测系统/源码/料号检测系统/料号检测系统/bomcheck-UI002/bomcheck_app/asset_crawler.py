...
class AssetCrawler:
    ...
    def _update_from_ua_sources(self, part_no: str) -> list[str]:
        ...
        if found_local:
            ...
            existing_local = [
                path
                for path in asset.local_paths
                if _clean_local_path(path, allow_relative=True)
            ]
            seen_local = set(existing_local)
            merged_local = list(existing_local)
            for path in found_local:
                if path not in seen_local:
                    merged_local.append(path)
                    seen_local.add(path)
            if merged_local != list(asset.local_paths):
        ...
        try:
            for sheet in workbook.worksheets:
                ...
                for raw_row in sheet.iter_rows(values_only=True):
                    row = list(raw_row or [])
                    detected = self._detect_ua_header(row)
                    if detected:
                        header, part_col, path_col = detected
                        continue
                    if header is not None and part_col is not None and path_col is not None:
                        cached = self._process_structured_ua_row(
                            row, header, part_col, path_col, index
                        )
                    else:
    ...
    def _detect_ua_header(self, values: Iterable) -> tuple[list[str], int, int] | None:
        header = [str(value or "").strip() for value in values]
        normalized = [_normalize_ua_header(value) for value in header]
        part_col = self._find_header_index(
            normalized,
            {"成品料号", "成品料號", "产品料号", "產品料號", "ua料号", "ua料號"},
        )
        path_col = self._find_header_index(
            normalized,
            {"档案位置", "檔案位置", "文件位置", "资料位置", "資料位置", "档案路径", "檔案路徑"},
        )
        if part_col is None or path_col is None:
            return None
        return header, part_col, path_col

    def _find_header_index(self, headers: list[str], candidates: set[str]) -> int | None:
        normalized_candidates = {_normalize_ua_header(item) for item in candidates}
    ...
    def _process_structured_ua_row(
        self,
        values: Iterable,
        header: list[str],
        part_col: int,
        ...
        row = list(values or [])
        if max(part_col, path_col) >= len(row):
            return None
        normalized_part = normalize_part_no(str(row[part_col] or ""))
        if not normalized_part:
            return None

        local_paths = self._extract_local_paths_from_values(
            [row[path_col]], allow_relative=False
        )
        if not local_paths:
            scan_values = [
                value
                for column, value in enumerate(row)
                if column < len(header) and not _is_ua_note_header(header[column])
            ]
            local_paths = self._extract_local_paths_from_values(
                scan_values, allow_relative=False
            )
        if not local_paths:
            return None

        combined_text = " ".join(str(value).strip() for value in row if value)
        index[normalized_part].update(local_paths)
    ...
    def _extract_local_paths_from_row(self, values: Iterable) -> list[str]:
        return self._extract_local_paths_from_values(values, allow_relative=True)

    def _extract_local_paths_from_values(
        self, values: Iterable, *, allow_relative: bool = False
    ) -> list[str]:
        paths: list[str] = []
        for value in values:
            if value is None:
            ...
            if not text or self._is_http_url(text):
                continue
            for candidate in _split_local_path_candidates(text):
                cleaned = _clean_local_path(candidate, allow_relative=allow_relative)
...
def _normalize_ua_header(value: str) -> str:
    return (
        str(value or "")
        .strip()
        .lower()
...
def _is_ua_note_header(value: str) -> bool:
    header = _normalize_ua_header(value)
    note_tokens = ("备注", "備註", "进度备注", "進度備註", "电控备注", "電控備註", "人力安排")
    return any(_normalize_ua_header(token) in header for token in note_tokens)


def _split_local_path_candidates(text: str) -> list[str]:
    # 只提取真正像 Windows 本地/共享目录的片段，避免把 3/5、5/28~6/5 这类备注当路径。
    pattern = r"(?:\\\\[^\r\n;；,，]+|[A-Za-z]:[\\/][^\r\n;；,，]+)"
    matches = re.findall(pattern, text)
    return matches or [text]


def _clean_local_path(text: str, *, allow_relative: bool = False) -> str | None:
    cleaned = str(text or "").strip().strip('"').rstrip("；;，,。")
    if not cleaned:
        return None
    ...
    is_drive = bool(re.match(r"^[A-Za-z]:[\\/]", cleaned))
    if is_unc or is_drive:
        return cleaned

    if allow_relative and ("\\" in cleaned or "/" in cleaned):
        # 兜底兼容旧数据中的相对路径，但必须排除普通备注/日期。
