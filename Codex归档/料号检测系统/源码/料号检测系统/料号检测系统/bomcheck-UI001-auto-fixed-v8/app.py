...
def apply_responsive_window_size(
    ...
    *,
    ...
    if parsed:
        width, height, fitted_min = _screen_limited_size(
            window, parsed[0], parsed[1], min_size=min_size, margin=margin
        )
        window.geometry(f"{width}x{height}")
        if fitted_min:
            window.minsize(*fitted_min)
    elif min_size:
        _, _, fitted_min = _screen_limited_size(
            window, min_size[0], min_size[1], min_size=min_size, margin=margin
        )
        if fitted_min:
            window.minsize(*fitted_min)
    try:
...
class Application:
    ...
    def _build_ui_v2(self) -> None:
        ...
        apply_responsive_window_size(self.root, "1180x760", (860, 560))
        ...
        sidebar = Frame(shell, bg=UI_COLORS["sidebar"], width=198)
        ...
        content_outer, content_stack, self._content_canvas = create_scrollable_content(
            workspace, UI_COLORS["bg"]
        )
        content_outer.pack(fill=BOTH, expand=True, padx=18, pady=18)
    ...
    def _build_config_page(self, parent: Frame) -> None:
        ...
        self._register_controlled_button(self.admin_button, "admin", {"side": LEFT, "padx": (0, 8), "pady": 5})
...
class DataFileEditor:
    ...
    def _build_ui_v2(self) -> None:
        ...
        apply_responsive_window_size(self.top, "980x660", (760, 520))
        self.top.configure(bg=UI_COLORS["bg"])

        header = Frame(self.top, bg=UI_COLORS["bg"])
        ...
        Label(
            header,
            text="集中维护公共配置中的数据源路径，并可处理系统料号原始文件。",
            bg=UI_COLORS["bg"],
            fg=UI_COLORS["muted"],
            font=("Microsoft YaHei UI", 10),
        ...
        Label(
            process_panel,
            text="将原始系统料号文件转换成标准 Excel，并同步生成快速加载 TSV。",
            bg=UI_COLORS["surface"],
            fg=UI_COLORS["muted"],
            justify="left",
            wraplength=300,
        ...
        Label(
            summary_panel,
            text="保存前建议先检查全部路径；缺失路径会在保存时被拦截。",
            bg=UI_COLORS["surface"],
            fg=UI_COLORS["muted"],
            justify="left",
            wraplength=300,
        ...
        button_frame = Frame(self.top, bg=UI_COLORS["bg"])
        button_frame.pack(fill="x", padx=18, pady=(12, 16))
        style_button(Button(button_frame, text="保存", command=self._on_save), "primary").pack(side=LEFT)
        style_button(Button(button_frame, text="取消", command=self.top.destroy), "secondary").pack(side=LEFT, padx=8)
...
class SystemPartViewer(Frame):
    ...
    def _build_ui(self) -> None:
        ...
        path_frame = Frame(main_frame, bg=UI_COLORS["surface"])
        ...
        tree_frame = Frame(main_frame, bg=UI_COLORS["surface"])
...
class LoginDialog:
    def __init__(self, master, store: AccountStore):
        ...
        prepare_dialog_window(self.top, "360x210")
...
class AdminSettingsDialog:
    def __init__(
        ...
        *,
        ...
        self.top.transient(master)
        self.top.grab_set()
        prepare_dialog_window(self.top, "760x520", (680, 480))
        self._dialog_kwargs = {"parent": self.top}
        self.username_var = StringVar()
        self.password_var = StringVar()
...
class BindingEditor:
    ...
    def __init__(
        ...
        *,
        ...
        self.top.transient(master)
        self.top.protocol("WM_DELETE_WINDOW", self._handle_close)
        prepare_dialog_window(self.top, "1080x700", (760, 500))
        self._dialog_kwargs = {"parent": self.top}
...
class PartAssetManager:
    ...
    def _build_ui(self) -> None:
        ...
        content_outer, main, self._content_canvas = create_scrollable_content(
            self.top, UI_COLORS["bg"]
        )
        content_outer.pack(fill=BOTH, expand=True, padx=10, pady=10)
        ...
        Entry(form, textvariable=self.model_var, width=34, state="readonly").grid(row=1, column=1, sticky="we", pady=(8, 0))
        ...
        image_scroll = Scrollbar(img_frame)
        image_scroll.pack(side=RIGHT, fill=Y)
        self.image_list = Listbox(img_frame, width=40, height=8, yscrollcommand=image_scroll.set)
        self.image_list.pack(side=LEFT, fill=BOTH, expand=True)
        image_scroll.config(command=self.image_list.yview)
        ...
        self.image_preview_label = Label(
            preview_frame, text="选择图片预览", anchor="center", width=22, height=8, wraplength=200
        )
