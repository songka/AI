...
from quotation.ui.widgets import (
    ...
    BatchQuotePage, HistoryPage, NavPanel, NewQuotePage,
    PlaceholderPage, PriceManagementPage, SettingsPage, SupplierManagementPage,
)
...
class DemoApp(tk.Tk):
    ...
    def _switch_page(self, name: str) -> None:
        ...
        if name == "新建報價":
            ...
            self._content = HistoryPage(self._content_area)
        elif name == "價格管理":
            self._content = PriceManagementPage(self._content_area)
        elif name == "供應商管理":
            self._content = SupplierManagementPage(self._content_area)
        elif name == "系統設定":
            self._content = SettingsPage(self._content_area)
        else:
