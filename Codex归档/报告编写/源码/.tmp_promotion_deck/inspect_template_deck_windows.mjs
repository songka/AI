...
import JSZip from "file:///C:/Users/lfaf-test/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/node_modules/jszip/lib/index.js";
...
import {
  ...
  requireArg,
  saveBlobToFile,
} from "file:///C:/Users/lfaf-test/.codex/plugins/cache/openai-primary-runtime/presentations/26.727.11326/skills/presentations/container_tools/artifact_tool_utils.mjs";
...
function zipNames(pptxPath) {
  return [...archiveEntries.keys()];
}

function readZipText(pptxPath, entryName) {
  return archiveEntries.get(entryName)?.toString("utf8") || "";
}

async function copyZipEntry(pptxPath, entryName, targetPath) {
  await fs.mkdir(path.dirname(targetPath), { recursive: true });
  await fs.writeFile(targetPath, archiveEntries.get(entryName) || Buffer.alloc(0));
}
  ...
  archiveEntries = new Map(
    await Promise.all(
      Object.entries(archive.files)
        ...
        .map(async ([name, entry]) => [name, await entry.async("nodebuffer")]),
    ),
  );
