...
SURFACE_KEYWORDS = [
    ...
    "滲碳", "渗碳", "carburizing",
    "RAL",
    "透明", "白色", "陽極", "阳极", "表面", "顏色", "颜色",
]

# Material keywords for detection priority
MATERIAL_KEYWORDS = [
    "S50C", "SKD11", "SKD61", "SUS304", "SUS316", "SPCC",
    "A6061", "AL6061", "6061", "6061-T6", "A5052",
    "SS400", "Q235", "45#", "45鋼",
    "普通鋼", "普通钢", "鋁", "铝", "銅", "铜",
    "不鏽鋼", "不锈钢",
]
...
class ManufacturingExtractor:
    ...
    def _extract_threads(
        ...
        for tc in texts:
            ...
            thread = ThreadFeature(
                ...
                source_entities=[tc.source_entity] + ([nearest_hole.source_entity] if nearest_hole and nearest_hole.source_entity else []),
    ...
    def _extract_surface_treatment(self, texts: list[TextCluster]) -> SurfaceTreatmentFeature | None:
    ...
    _ACCESSORY_KW = {
        "合頁": "DOOR_HARDWARE", "鉸鏈": "DOOR_HARDWARE", "磁吸": "DOOR_HARDWARE",
        "把手": "DOOR_HARDWARE", "門鎖": "DOOR_HARDWARE", "拉手": "DOOR_HARDWARE",
        "角碼": "FASTENER", "角件": "FASTENER",
    }
    _ASSEMBLY_KW = {
        "防護": "GUARD", "圍欄": "GUARD", "護罩": "GUARD",
        "機架": "FRAME", "框架": "FRAME",
        "門": "DOOR", "門組": "DOOR",
        "罩": "ENCLOSURE",
    }

    def _extract_frames(self, geo: GeometricFeatures) -> list[FrameFeature]:
        for tc in geo.text_clusters:
            for kw in self._FRAME_KW:
                if kw in tc.content:
                    length = geo.bounding_box.length * 4 if geo.bounding_box else 0
                    ...
                    return [FrameFeature(
                        ...
                        connection_type="角碼" if any("角碼" in t.content or "角件" in t.content for t in geo.text_clusters) else None,
            ...
            for kw in self._ACRYLIC_KW:
                if kw in tc.content:
                    ...
                    return [AcrylicFeature(
                        ...
                        material="亞克力" if any(k in tc.content for k in ("亞克力", "壓克力")) else "PC",
                        area_mm2=area,
                        color="白色透明" if "透明" in tc.content and "白" in tc.content else None,
                        source_entities=[tc.source_entity] if tc.source_entity else [],
                        confidence=0.75,
                    )]
        return []
    ...
    def _extract_accessories(self, geo: GeometricFeatures) -> list[StructureAccessoryFeature]:
        ...
        if not items:
            return []
        return [StructureAccessoryFeature(
            accessory_id=f"ACC-{uuid.uuid4().hex[:6]}",
            category="DOOR_HARDWARE" if any(k in items for k in ("合頁", "鉸鏈", "磁吸", "把手", "門鎖", "拉手")) else "FASTENER",
            items=items,
            source_entities=handles,
            confidence=0.75,
        )]
        ...
        for tc in geo.text_clusters:
            for kw, atype in self._ASSEMBLY_KW.items():
                if kw in tc.content and atype not in seen:
                    seen.add(atype)
                    assemblies.append(StructureAssemblyFeature(
