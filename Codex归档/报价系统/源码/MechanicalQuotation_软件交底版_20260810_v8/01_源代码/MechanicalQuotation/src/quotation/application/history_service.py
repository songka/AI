...
from quotation.application.runtime_paths import is_shared_database, resolve_database_address
...
def _db_path() -> Path:
    """Resolve the configured local or shared database address."""

    address = UserSettingsService().load().get("database_address")
    path = resolve_database_address(address)
...
class QuotationHistory:
    ...
    def __init__(self, db_path: str | Path | None = None):
        self._db_path = resolve_database_address(db_path) if db_path else _db_path()
        self._db_path.parent.mkdir(parents=True, exist_ok=True)
        self._init_db()

    def _connect(self) -> sqlite3.Connection:
        """Open a connection that tolerates short concurrent-write windows."""

        conn = sqlite3.connect(str(self._db_path), timeout=30.0)
        conn.execute("PRAGMA busy_timeout = 30000")
        conn.execute(
            "PRAGMA journal_mode = DELETE"
            if is_shared_database(self._db_path)
    ...
    def _recalculate_quote(
        ...
        ai_count = sum(row["source"] == "AI" for row in items)
