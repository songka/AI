...
from quotation.ui.widgets import (
    ...
    BatchQuotePage, DatePickerEntry, ManagementPage, NavPanel, NewQuotePage,
    StructuredDetailWindow, SystemSettingsPage, quote_detail_sections,
...
def quote_feature_display_fields(summary: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    weight_names = {
        ...
        "CONFIRMED_PART_DIMENSION": "按图纸明确的零件规格计算",
        "CONFIRMED_SHEET_DIMENSION_AND_THICKNESS": "按薄板规格与厚度计算",
        "MISSING_CONFIRMED_DIMENSION": "缺少可靠三维规格，待确认",
        "MISSING_CONFIRMED_SHEET_DIMENSION": "缺少可靠薄板长宽，待确认",
        "DRAWING_EXTENT_SHEET_ESTIMATE": "按绘图范围临时估算，需复核",
...
class DemoApp(tk.Tk):
    ...
    def _switch_page(self, name: str) -> None:
        """Switch pages while retaining the last single and batch quote views."""
        if name == "管理员登录":
        ...
        if name == "退出登录":
            ...
            return
        persistent_pages = {"新建报价", "批量报价"}
        if self._content is not None:
            if self._current_page_name in persistent_pages:
                self._content.pack_forget()
            else:
                self._content.destroy()
        cached = self._page_cache.get(name)
        if cached is not None and cached.winfo_exists():
            self._content = cached
        ...
        if name in persistent_pages:
            self._page_cache[name] = self._content
        self._current_page_name = name
        self._content.pack(fill=tk.BOTH, expand=True)
    ...
    def _maintenance_form(
        ...
        choice_values: dict[str, dict[str, str]] = {}
        ...
        for index, field in enumerate(fields):
            if isinstance(field, dict):
                ...
                key, label = field
                kind = "password" if key == "password" else "entry"
                options = []
                default = ""
            tk.Label(window, text=label).grid(row=index, column=0, sticky=tk.W, padx=12, pady=6)
            raw_value = (initial or {}).get(key)
            variable = tk.StringVar(
                value=str(raw_value if raw_value not in (None, "") else default)
            )
            variables[key] = variable
            if kind == "date":
                widget: tk.Widget = DatePickerEntry(
                    window, textvariable=variable, width=27
                )
            elif kind in {"combo", "editable_combo"}:
                if isinstance(options, dict):
                    display_to_value = {
                        str(name): str(value) for name, value in options.items()
                    }
                    choice_values[key] = display_to_value
                    current_value = variable.get()
                    variable.set(
                        next(
                            (
                                name
                                for name, value in display_to_value.items()
                                if value == current_value
                            ),
                            current_value,
                        )
                    )
                    option_names = list(display_to_value)
                else:
                    option_names = [str(value) for value in options]
                widget = ttk.Combobox(
                    window,
                    values=option_names,
                ...
                if kind == "combo" and not variable.get() and option_names:
                    variable.set(option_names[0])
            else:
                widget = tk.Entry(
                    window,
                    textvariable=variable,
            ...
            widget.grid(row=index, column=1, sticky=tk.EW, padx=12, pady=6)
            widgets[key] = widget
        window.columnconfigure(1, weight=1)

        for definition in dynamic_fields:
            target_key = str(definition["key"])
            source_key = str(definition.get("depends_on") or "")
            if not source_key or source_key not in widgets:
                continue
            ...
            def update_dependent(
                ...
                *,
                ...
                spec: dict[str, Any] = definition,
            ) -> None:
                source_display = variables[source].get().strip()
                source_value = choice_values.get(source, {}).get(
                    source_display, source_display
                )
                options_by = spec.get("options_by") or {}
                if source_value in options_by and isinstance(
                    widgets[target], ttk.Combobox
                ):
                    widgets[target].configure(values=options_by[source_value])
                default_by = spec.get("default_by") or {}
                if source_value in default_by:
                    default_value = str(default_by[source_value])
                    variables[target].set(
                        next(
                            (
                                name
                                for name, value in choice_values.get(target, {}).items()
            ...
            widgets[source_key].bind("<<ComboboxSelected>>", update_dependent)
            update_dependent()
        ...
        def confirm() -> None:
            ...
            for key, variable in variables.items():
                entered = variable.get().strip()
                result[key] = choice_values.get(key, {}).get(entered, entered)
            window.destroy()
    ...
    def _edit_supplier(self, row: dict[str, Any] | None) -> None:
        ...
        values = self._maintenance_form(
            ...
            [("supplier_name", "供应商名称"), ("contact_person", "联系人"),
             ("phone", "电话"),
             {"key": "quality_rating", "label": "质量等级", "kind": "combo",
              "options": ["A", "B", "C"]},
             ("notes", "备注")],
            row,
        )
    ...
    def _add_supplier_price(self, row: dict[str, Any] | None) -> None:
        ...
        values = self._maintenance_form(
            ...
            [{
                ...
                "options_by": {
                    "MATERIAL": ["S50C", "A6061-T6", "SUS304", "SPCC", "SKD11"],
                    "PROFILE": ["铝型材", "方管", "角钢", "钢板"],
                    "PROCESS": ["CNC", "车床", "铣床", "磨床", "钳工"],
                    "SURFACE": ["镀铬", "阳极氧化", "发黑", "喷涂"],
                },
                "options": [
                    "S50C", "A6061-T6", "SUS304", "SPCC", "SKD11",
                    "CNC", "车床", "铣床", "磨床", "钳工",
                    "镀铬", "阳极氧化", "发黑",
                ],
             },
             ("material_spec", "规格（可选）"),
             ("unit_price", "未税单价（未知请留空）"),
             {
                "key": "unit",
                "label": "单位",
                "kind": "combo",
                "options": {
                    "千克 kg": "kg", "件 piece": "piece", "小时 hour": "hour",
                    "米 m": "m", "平方米 m²": "m2", "项 item": "item",
                },
                "default": "kg",
                "depends_on": "target_type",
                "default_by": {
                    "MATERIAL": "kg", "PROFILE": "m", "PROCESS": "hour",
                    "SURFACE": "m2",
                },
             }, {
                "key": "effective_from",
    ...
    def _open_manual_review(self, row: dict[str, Any]) -> None:
        ...
        line_ids = [str(item["line_id"]) for item in detail["items"]]
        ...
        for index, (label, key) in enumerate([
            ...
            if key == "field":
                ...
                widget = ttk.Combobox(window, values=line_ids, state="normal", width=38)
            else:
                widget = tk.Entry(window, width=42)
                if key == "operator":
                    widget.insert(0, self._session.username)
            widget.grid(row=index, column=1, padx=10, pady=6)
            entries[key] = widget
        tk.Label(window, text="费用行可直接从上方下拉选择", fg="#7f8c8d").grid(
    ...
    def _run_selected_quote(self) -> None:
        ...
        def report_progress(message: str, fraction: float) -> None:
            self.after(
                0,
                lambda m=message, f=fraction: page.update_status(
                    m, "orange", max(0, min(f * 100, 99))
                ) if page.winfo_exists() else None,
            )
    ...
    def _batch_run(
        ...
        results = svc.quote_batch(
            bundles, use_ai=use_ai, progress_callback=progress_callback
        )
