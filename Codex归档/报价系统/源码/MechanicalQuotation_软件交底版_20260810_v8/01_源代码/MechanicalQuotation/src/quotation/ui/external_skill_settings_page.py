...
class ExternalSkillSettingsPage(tk.Frame):
    ...
    def _build(self) -> None:
        ...
        tk.Button(row, text="检测 Skill", command=self._discover).pack(side=tk.LEFT, padx=5)
        tk.Button(row, text="载入一组 Skill", command=self._discover_group).pack(side=tk.LEFT, padx=5)
        tk.Button(row, text="检测外挂智能体", command=self._discover_agent).pack(side=tk.LEFT, padx=5)
        tk.Button(row, text="移除选中资源", command=self._remove).pack(side=tk.LEFT, padx=5)
        ...
        tk.Label(
            registry,
            text="提示：用上方四页切换资源；双击可看能力、正文和文件结构，非文本文件只展示类型与大小。",
            bg=CARD_BG,
            fg="#566573",
            font=_font(9),
    ...
    def _apply_process_route(self) -> None:
        ...
        code = self._selected_process_code()
        if step_routes:
    ...
    def _selected_resource_key(self, tree: ttk.Treeview | None = None) -> str | None:
        candidates = [tree] if tree is not None else list(self._resource_trees.values())
