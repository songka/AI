...
`company_price_id/price_version_id`，单价完全一致，`amount=quantity×unit_price`。历史整件价格不得
匹配。无正式价可返回 `source=AI`、`AI_REFERENCE`、`requires_review=true`、
`included_in_quotation=true` 并计入本次报价；不得用整件参考价覆盖分项或冒充 C 价。
