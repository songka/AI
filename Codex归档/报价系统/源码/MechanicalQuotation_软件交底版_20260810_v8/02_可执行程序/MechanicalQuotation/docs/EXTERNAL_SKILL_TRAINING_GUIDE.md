...
`DOCUMENT_UNDERSTANDING` 和 `PART_CLASSIFICATION` 都是分类前的全局步骤，不得按零件类别切换 Skill。
第 2 步输出 `MACHINING`、`SHEET_METAL`、`WELDMENT` 或 `FRAME_ASSEMBLY`。主程序校验分类与置信度后，
从 `FEATURE_EXTRACTION` 开始的第 3～11 步才选择对应类别路由；分类无效时回退内置规则。
整套 Skill 模式也遵守同一边界：全局整套 Skill 执行第 1、2 步，类别专属整套 Skill 只接管第 3～11 步。
