...
RESOLUTION_DISPLAY: dict[str, str] = {
    ...
    "AI_ESTIMATE_INCLUDED_REVIEW_REQUIRED": "AI估算已計入，待人工確認",
    "EXTERNAL_SKILL_AI_INCLUDED_REVIEW_REQUIRED": "外接 Skill AI估算已計入，待人工確認",
}
...
class QuoteItemViewModel:
    ...
    @property
    def is_ai_priced(self) -> bool:
        return self.item.source == PriceSource.AI
    ...
    def display_ai_estimate(self) -> str:
        """AI estimate included in the quote and awaiting human confirmation."""
        if self.item.ai_estimated_amount is None:
            return "—"
        return f"¥{self.item.ai_estimated_amount:,.2f}（已计入，待确认）"
    ...
    def status_label(self) -> str:
        if self.is_unknown:
            return "待确认"
        if self.is_ai_priced:
            return "AI估算已计入，需人工确认"
        return "已确认"
    ...
    def row_tags(self) -> list[str]:
        return ["unknown"] if self.is_unknown or self.is_ai_priced else []
    ...
    def trace_fields(self) -> list[tuple[str, str]]:
        ...
        if self.item.ai_estimated_amount is not None:
            fields.append(
                ("智能辅助估算总额", f"¥{self.item.ai_estimated_amount:,.2f}（已计入，需人工确认）")
            )
            fields.append(
                (
                    "智能辅助参考单价",
            ...
            fields.append(("智能辅助估价依据", self.item.ai_estimate_reason or "AI估算，需人工确认"))
            fields.append(
