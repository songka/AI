...
| `PROCESS_PLANNING` | 工艺路线 | 最低成本可完成设备、工序顺序 | 不默认全部使用 CNC |
| `TIME_ESTIMATION` | 工时估算 | 准备、装夹、加工、检验工时及依据 | 不提供正式小时费率 |
| `LINE_ITEM_PRICING` | 分项计价 | 引用正式价格、计算数量与金额 | 不伪造 company_price_id |
| `UNKNOWN_ESTIMATION` | 待确认估价 | U 项参考区间、假设、可信度 | 不进入正式总价 |
...
AI 推测价只能作为待确认参考，不得计入正式总价。信息不足时返回待确认，不得编造。
```
