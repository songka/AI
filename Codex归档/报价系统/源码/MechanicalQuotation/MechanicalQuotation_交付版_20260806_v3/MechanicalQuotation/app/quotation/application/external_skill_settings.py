...
class ExternalSkillRoutingConfig(BaseModel):
    ...
    def validate_routes(self):
        ...
        if self.mode == SkillRoutingMode.FULL_QUOTATION:
            if not self.full_skill_id:
                raise ValueError("整套报价模式必须选择一个 Skill")
            selected = skills.get(self.full_skill_id)
            if selected is None or not selected.supports_full_quotation:
                raise ValueError("整套报价只能选择已启用且声明支持整套报价的 Skill")
            if self.step_routes:
                raise ValueError("整套报价模式不能同时配置分步路由")
        else:
            if self.full_skill_id is not None:
                raise ValueError("分步模式不能设置整套报价 Skill")
            for step, route in self.step_routes.items():
                if route.provider == "builtin":
                    continue
                selected = skills.get(route.provider)
                if selected is None:
                    raise ValueError(f"步骤“{STEP_NAMES_ZH[step]}”引用了未启用的 Skill")
                if step not in selected.supported_steps:
                    raise ValueError(
                        f"Skill“{selected.name_zh}”不支持步骤“{STEP_NAMES_ZH[step]}”"
                    )
        return self

    def provider_for(self, step: SkillStep) -> str:
        if self.mode == SkillRoutingMode.FULL_QUOTATION:
            return self.full_skill_id or "builtin"
        return self.step_routes.get(step, StepRoute()).provider
...
class ExternalSkillSettingsStore:
    ...
    def load(self) -> ExternalSkillRoutingConfig:
        if self.sync_enabled and self.primary_path.is_file():
