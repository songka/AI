        ...
        try:
            ...
            if use_ai and self._ai_client is not None:
                try:
                    ...
                    drawing_texts = drawing_text_context
                    explicit_cnc = any(
                    ...
                    if explicit_cnc and "銑床" in ai_process_names:
                        ...
                        alternatives = {
                            ...
                            for process in ai_processes
                            if process["process_name"] in {"CNC", "銑床"}
                        }
                        ...
                        for process_name, process in alternatives.items():
                            ...
                            if rate is not None and rate.unit_price > 0:
                                evaluated[process_name] = round(
                                    process["estimated_hours"] * rate.unit_price, 2
                                )
                        if len(evaluated) == 2:
                            ...
                            ai_processes = [
                                process
                                for process in ai_processes
                                if process["process_name"] != rejected_name
                            ]
                            if selected_name == "銑床":
                                items = [
                                    item
                        ...
                        if len(items) < original_count:
                            selected = next(
                                process for process in ai_processes
                                if process["process_name"] in {"銑床", "車床"}
                            )
                            selected_rate = resolver.lookup(
                                "process", selected["process_name"]
                            )
                            selected_cost = (
                                selected["estimated_hours"] * selected_rate.unit_price
