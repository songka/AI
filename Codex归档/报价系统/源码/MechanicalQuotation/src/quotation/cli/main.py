...
def skill_command(
    ...
    try:
        ...
        if task_type in {SkillTaskType.EXCEL_WRITE, SkillTaskType.EXCEL_EXPORT} and not output_excel:
            raise ValueError(f"{task_type.value} 必须提供 --output-excel")
        if task_type == SkillTaskType.EXCEL_MODIFY and (not input_excel or not output_excel):
            raise ValueError("EXCEL_MODIFY 必须同时提供 --input-excel 和 --output-excel")
