...
try {
    ...
    if (-not (Test-Path -LiteralPath $readyMarker)) {
        ...
        Get-ChildItem -LiteralPath $versionsRoot -Directory -Filter (".staging-" + $versionHash + "-*") -ErrorAction SilentlyContinue |
            Remove-Item -Recurse -Force
        Get-ChildItem -LiteralPath $versionsRoot -File -Filter ("payload-" + $versionHash + "-*.zip") -ErrorAction SilentlyContinue |
            Remove-Item -Force
        ...
        if ($payloadMode) {
            ...
            & tar.exe -xf $localPayload -C $stagingRoot
            if ($LASTEXITCODE -ne 0) {
                throw "Extracting the local application package failed. Tar exit code: $LASTEXITCODE"
            }
